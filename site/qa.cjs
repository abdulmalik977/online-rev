...
(async()=>{
 ...
 for(const width of [320,390,430,1440]){
  ...
  for(const page of ['index.html','privacy.html','unsubscribe.html']){
   await send('Page.navigate',{url:'http://127.0.0.1:8777/'+page});
   let info;
   for(let i=0;i<40;i++){
    const r=await send('Runtime.evaluate',{expression:`JSON.stringify({ready:document.readyState,path:location.pathname,width:innerWidth,scroll:document.documentElement.scrollWidth,checkout:document.querySelector('.price-card button')?.disabled,heading:document.querySelector('h1')?.textContent,load_ms:performance.getEntriesByType('navigation')[0]?.loadEventEnd,cookies:document.cookie})`,returnByValue:true});
   ...
   if(page==='index.html'&&[390,1440].includes(width)){
    ...
    fs.writeFileSync('.runtime/task007-qa/'+width+'.png',Buffer.from(shot.data,'base64'));
   }
  }
 }
 fs.writeFileSync('.runtime/task007-qa/layout.json',JSON.stringify(results,null,2));
 const failed=results.filter(x=>x.width!==x.viewport||x.scroll>x.width||!x.heading||x.cookies||x.page==='index.html'&&!x.checkout);
